...
const DB = {
  ...
  async init() {
    try {
      const [profile, workouts, weights, todayDiet, trainingPlans, perfLogs, dietTemplate] = await Promise.all([
        ...
        _api('/api/diet-template').then(r => r.json()),
      ]);
      ...
      cache.dietTemplate = dietTemplate || { meals: { breakfast: [], lunch: [], dinner: [], snack: [] } };
    } catch (e) {
      ...
      cache.dietTemplate  = JSON.parse(localStorage.getItem('dietTemplate'))
  ...
  // ── Diet Template ──────────────────────────────────────────────────────────
  getDietTemplate() {
    return cache.dietTemplate || { meals: { breakfast: [], lunch: [], dinner: [], snack: [] } };
  },
  async saveDietTemplate(data) {
    cache.dietTemplate = data;
    localStorage.setItem('dietTemplate', JSON.stringify(data));
    try {
      await _api('/api/diet-template', { method: 'PUT', body: JSON.stringify(data) });
